...
while (("$#")); do
  case "${1}" in
    ...
    --description=*)
      embed_description=${1/--description=/''}
      embedding=1
      shift
      ;;
    --description*)
    ...
    --author-icon=*)
      embed_authoricon=${1/--author-icon=/''}
      embedding=1
      shift
      ;;
    --author-icon*)
    ...
    # image
    --image-height=*)
      embed_imageheight=${1/--image-height=/''}
      embedding=1
      shift
      ;;
    --image-height*)
    ...
    --image-width=*)
      embed_imagewidth=${1/--image-width=/''}
      embedding=1
      shift
      ;;
    --image-width*)
    ...
    # footer
    --footer-icon=*)
      embed_footericon=${1/--footer-icon=/''}
      embedding=1
      shift
      ;;
    --footer-icon*)
...
send() {
  ...
  if [[ -n "${username}" ]] || [[ -n "${avatar_url}" ]] && [[ "${modify}" = 1 ]]; then
    if [[ -n "${text}" ]] && [[ -n "${embed_title}" ]] && [[ -n "${embed_description}" ]] && [[ -n "${embed_url}" ]] && [[ -n "${embed_color}" ]] && [[ -n "${embed_timestamp}" ]] && [[ -n "${embed_authorurl}" ]] && [[ -n "${embed_authoricon}" ]] && [[ -n "${embed_authorname}" ]] && [[ -n "${embed_thumbnail}" ]] && [[ -n "${embed_imageheight}" ]] && [[ -n "${embed_imagewidth}" ]] && [[ -n "${embed_imageurl}" ]] && [[ -n "${embed_footertext}" ]] && [[ -n "${embed_footericon}" ]] && [[ -n "${fields}" ]] && [[ -n "${file_path}" ]]; then
      echo "fatal: no arguments specified (except for username, modify and avatar)"
      exit 1
    fi
